...
module Sunspot
  ...
  module Adapters
    ...
    class InstanceAdapter
      ...
      #
      def index_id #:nodoc:
        InstanceAdapter.index_id_for(Apartment::Tenant.current, @instance.class.name, id)
      end
      ...
      class <<self
        ...
        def index_id_for(tenant_name, class_name, id) #:nodoc:
          "#{tenant_name} #{class_name} #{id}"
        end
      ...
      def inject_inherited_attributes_for(data_accessor)
        ...
        data_accessor.inherited_attributes.each do |attribute|
          if data_accessor.send(attribute).nil? # Inject only if the current class didn't define one.
            ...
            data_accessor.send("#{attribute}=", inherited_value) unless inherited_value.nil?
          end
        end
        data_accessor
      end
