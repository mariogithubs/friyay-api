...
module Sunspot
  ...
  class Indexer #:nodoc:
    ...
    def remove_by_id(class_name, *ids)
      ids.flatten!
      @connection.delete_by_id(
        ids.map { |id| Adapters::InstanceAdapter.index_id_for(Apartment::Tenant.current, class_name, id) }
      )
    end
    ...
    def prepare_full_update(model)
      ...
      # ::Rails.logger.debug "===> SOLR Update (full) class: #{model.class} - id: #{model.id}"
      document
    end
    ...
    def prepare_atomic_update(clazz, id, updates = {})
      ...
      setup_for_class(clazz).all_field_factories.each do |field_factory|
        if updates.key?(field_factory.name)
          field_factory.populate_document(document, nil, value: updates[field_factory.name], update: :set)
        end
      end
      # ::Rails.logger.debug "===> SOLR Update (atomic) clazz: #{clazz} - id: #{id} - updates: #{updates}"
      document
    end
    ...
    def document_for(clazz, id)
      if Adapters::InstanceAdapter.for(clazz)
        RSolr::Xml::Document.new(
          id: Adapters::InstanceAdapter.index_id_for(Apartment::Tenant.current, clazz.name, id),
    ...
    def setup_for_class(clazz)
      Setup.for(clazz) || fail(NoSetupError, "Sunspot is not configured for #{clazz.inspect}")
    end
  end
end
