...
class SlackLink < Attachment
  include Slack::History
  ...
  def self.add(tip, urls)
    urls.each do |url|
      attachment = tip.attachments.find_by_file_and_type(url, 'SlackLink')
      urls.each { |url| tip.attachments.create(file: url, type: 'SlackLink') } unless attachment.present?
    end
  end
